...
const Item = ({ id, index, item, itemActions, moveCard }: ItemProps) => {
    ...
    return (
        <RowItem ref={ref} style={{ opacity }} data-handler-id={handlerId}>
            <Name className={item.completed ? 'done' : ''}>{ item.text }</Name>
            <Actions>
                {
                    item.completed ? 
                        <FaCheckSquare 
